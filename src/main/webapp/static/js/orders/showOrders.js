layui.use(['jquery','layer', 'table','form','laydate'], function() {
    ...
    var selJsonOrders = {};  //全局的条件查询变量Json对象

    var currentPage = 1;    //定义全局的当前页

    //初始化订单分页数据
    loadOrdersByPramas(selJsonOrders);
    ...
    form.on('submit(demo1)', function(data){
        ...
        loadOrdersByPramas(selJsonOrders);  //根据条件查询订单数据
        return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
    });

    //直接使用layui的表格分页加载订单数据，自定义的函数，必须调用才能执行
    function loadOrdersByPramas(selJsonOrders) {
        //表格的分页加载，数据表格方法级渲染
        table.render({  //数据表格的数据渲染(此UI框架底层是进行异步加载)
            ...
            , url: 'orders/loadPageTByPramas' //访问服务器端的数据接口(异步请求)，返回的json格式的数据
    ...
    table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'del') { //删除
            ...
            layer.confirm('真的确定支付此订单信息吗？', function (index) {
                //向服务端发送支付指令，开启一个浏览器窗口，访问服务器，跳转到支付页面
        ...
        $.ajax({
            ...
            success:function (data) {
                if(data=='updSuccess'){
                    ...
                    layer.msg("订单数据批量删除失败",{icon: 2,time:2000,anim: 5,shade:0.5});
                }
            },
            error:function () {
                layer.msg("服务器异常！！！",{icon: 3,time:2000,anim: 6,shade:0.5});
